...
if(request_method() === "POST" && user_is_logined() && user_is_student()) { // VALIDA SE O MÉTODO ENVIADO ESTÁ CORRETO E SE O USUÁRIO (ALUNO) ESTÁ LOGADO NO SISTEMA
	...
	$stage = step_identifier(FORM_STAGES[5], 6);

	// CRIA UM FORMULÁRIO EM BRANCO
	$questions = medium_english_form($empty=true);

	// VALIDADOR SE DEVE SALVAR E SAIR
	$logout = isset($_POST["logout"]) ? (bool) clean_text($_POST["logout"]) : false;

	if($level["module"] === FORM_MODULES[2] && !$level["started"]) { // USUÁRIO DESEJA INICIAR O FORMULÁRIO (NÍVEL DE INGLÊS - INGLÊS MÉDIO)
		// INSERE AS RESPOSTAS EM CADA PERGUNTA CORRESPONDENTE
		$response = fills_in_empty_responses($questions, $questions);

		// INSERE NO BANCO DE DADOS QUE O USUÁRIO INICIOU O PREENCHIMENTO DO NÍVEL DE INGLÊS - INGLÊS MÉDIO
		$rows = insert_form($stage, $response);

		if($rows) { // GRAVA A OPERAÇÃO DE INSERÇÃO NO LOG
			record_log("Student (" . get_user()["id"] . "-" . get_user()["student"] . ":" . get_user()["name"] . ") started the form 'English Level - Medium English' (" . $stage . ":" . $response . ")");
		}
	}

	elseif($level["module"] === FORM_MODULES[2] && !$level["finished"]) { // USUÁRIO INICIOU O FORMULÁRIO (NÍVEL DE INGLÊS - INGLÊS MÉDIO) E DESEJA TERMINAR A ETAPA OU SAIR
		// INSERE AS RESPOSTAS EM CADA PERGUNTA CORRESPONDENTE
		...
		record_log("Student (" . get_user()["id"] . "-" . get_user()["student"] . ":" . get_user()["name"] . ") " . ($logout ? "changed" : "finished") . " the form 'English Level - Medium English' (" . $stage . ":" . $response . ")");
	}
}
...
else { // REDIRECIONA PARA A PÁGINA PRINCIPAL, QUE IRÁ REDIRECIONAR PARA A PÁGINA CORRETA (PRÓXIMA PÁGINA: AVALIAÇÃO COMPORTAMENTAL)
	redirect(BASE_NAME);
}
